...
function Invoke-Runspace_Procedure {
<#
.SYNOPSIS
    This function allows the invoke scriptblock to run within a new runspace.
...
#>
    ...
    param(
        ...
        [PSCustomObject]$CommandType,
        [Parameter(Position=4,Mandatory=$True)]
        [PSCustomObject]$ProcedureMethod,
        [Parameter(Position=5,Mandatory=$True)]
        [PSCustomObject]$ProcedureInput,
        [Parameter(Position=6,Mandatory=$True)]
        [PSCustomObject]$ProcedureDateTime,
                    ...
                    if($Items.Count -ge 1){
                        ...
                        if($ProcedureMethod -eq 'Test-Path'){
                            if(Test-Path $ProcedureInput){
                                ...
                                if($ProcedureGci.LastWriteTime -gt $ProcedureDateTime){
                            ...
                            Write-Warning 'Procedure method is not exist!'
                        }
                    }
                    else{
                        $RunspaceProcessDetail.Status = 'Done'
